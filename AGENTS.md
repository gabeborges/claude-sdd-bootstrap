...
Workflow: Spec-Driven Development (SDD) using Clavix + Agent Swarm (no OpenSpec).
...
1. `.ops/product-vision-strategy.md`
   - High-level product context generated by Clavix.

2. `.ops/ui-design-system.md` (when UI exists)
...
3. `.ops/build/system-design.yaml`
   - Project-level system design plan.

4. `.ops/build/v{x}/prd.md`
   - Build/version scope and acceptance intent for the current version.

5. `.ops/build/v{x}/<feature-name>/specs.md`
   - Feature requirements + acceptance criteria (scenarios). **This is the acceptance source of truth**.

6. `.ops/build/v{x}/<feature-name>/tasks.md`
   - Feature tickets derived from `specs.md`.
   - Each ticket MUST include `implements:` pointers into `specs.md`.


### Strict definitions (do not drift)
- `tasks.md` = feature tickets in `.ops/build/v{x}/<feature-name>/tasks.md`
- `specs.md` = feature requirements + acceptance criteria in `.ops/build/v{x}/<feature-name>/specs.md`
- `prd.md` = build-level plan in in `.ops/build/prd.md`
- `implementation-status.md` = build-level progress tracker `.ops/build/implementation-status.md`
...
└── build/    
    ...
    └── v{x}/
        ...
        └── <feature-name>/
            ├── specs.md
            ├── tasks.md
            └── (optional) checks.yaml db-migration-plan.yaml, spec-change-requests.yaml, etc.
```
...
- `specs.md` is missing, contradictory, or lacks scenarios that define acceptance.
- `tasks.md` is missing or any ticket lacks a valid `implements:` pointer into `specs.md`.
...
- Do not disable lint rules or introduce ignores unless explicitly required by tasks/spec and documented in `.ops/build/decisions-log.md`.
...
- Which `specs.md` requirements/scenarios were satisfied (via `implements:` pointers)
...
| Spec Writer | Spec authoring + feature breakdown| planning | [.claude/agents/spec-writer.md](.claude/agents/spec-writer.md) |
...
### Planning
- **Project Task Planner** — Parses OpenSpec into feature `tasks.md` with `implements:` pointers into feature `specs.md`
...
Tier 1: workflow-orchestrator, context-manager
   ↓
Tier 2: spec-writer → project-task-planner
   ↓
...
The workflow-orchestrator spawns agents tier-by-tier and auto-detects which optional agents are needed from `tasks.md` / `specs.md` content.
...
| Knowledge Synthesizer | Deviation logger + build knowledge compactor | orchestration | [agents/knowledge-synthesizer.md](agents/knowledge-synthesizer.md) |
